/**
 * test.mjs — Manual test suite for csv-array (testing from npm)
 *
 * Run with:  node test.mjs
 */

import { parseCSV } from "csv-array";
import path from "path";
import fs from "fs";

// Adjust path to parent data.csv
const dataCsvPath = path.resolve(process.cwd(), "../data.csv");
...
section("1. parseCSV Promise — with headings (data.csv > 10 MB → Worker thread)");

try {
  const rows = await parseCSV(dataCsvPath);
...
try {
  const rows = await parseCSV(dataCsvPath, undefined, false);
...
try {
  const rows = await parseCSV(dataCsvPath, undefined, true, { start: 10, count: 5 });
